use apollo_compiler::{ast::{Definition, Type}, parser::Parser};
use rmcp::schemars::schema::{ArrayValidation, InstanceType, ObjectValidation, RootSchema, Schema, SchemaObject, SingleOrVec, SubschemaValidation};
 ...
 * - error handling
 */
pub fn operation_to_json_schema(uri: &str, source_text: &str) -> RootSchema {
    let document = Parser::new().parse_ast(source_text, uri).expect("failed to parse operation");
    let operation_variables = document.definitions.iter().filter_map(|def| match def {
        Definition::OperationDefinition(operation_def) => Some(operation_def.variables.clone()),
        Definition::FragmentDefinition(_) => None,
        _ => panic!("Schema definitions were passed in, only operations and fragments are allowed"),
    });

    let operation_count = operation_variables.clone().count();
    assert!(operation_count <= 1, "too many operations in document: {operation_count}");
...
mod tests {
    ...
    use crate::operation_to_json_schema::operation_to_json_schema;

    #[test]
    fn no_variables() {
        let root_schema = operation_to_json_schema("operation.graphql", "query { id }");
        assert_eq!(json!(root_schema), json!({"type": "object"}))
    }

    #[test]
    fn nullable_named_type() {
        let root_schema = operation_to_json_schema("operation.graphql", "query($id: ID) { id }");
        assert_eq!(
            json!(root_schema),
            json!({
                "type": "object",
