...
From the root of a local MCP server repo, run the `mcp-apollo-server` binary with the example persisted query manifest, `graphql/weather/persisted_queries/apollo.json`:
...
docker run \
  ...
  --sse-port 5000 \
  -s api.graphql \
  -o forecast.graphql alerts.graphql all.graphql
```
