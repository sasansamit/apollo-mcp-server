...
1. [Install the MCP server](https://www.apollographql.com/docs/apollo-mcp-server/command-reference#installation).
...
__In addition to the standalone container we also provide the Apollo Runtime Container that provides all the necessary components to run the Apollo Runtime, including the Apollo Router and the MCP Server.
For more information, including how to deploy and configure this container, please see the [Apollo Runtime Container repository](https://github.com/apollographql/apollo-runtime).__
