...
The bulk of the setup is done in the Identity Provider. The MCP server and the GraphOS Router are then configured to use this IdP for authorization. 
...
In this guide, you'll use [Auth0](https://auth0.com/) as the Identity Provider.
...
1. Run the following curl command to promote the connection to domain-level, replacing `CONNECTION_ID` and `MGMT_API_ACCESS_TOKEN` with the values you copied earlier:
...
1. Paste the following configuration, replacing `<AUTH0 DOMAIN>` with your Auth0 domain:
    ...
    With this configuration, the router requires authentication for all requests. If no Authorization token is provided, the router returns an `UNAUTHENTICATED` error.
