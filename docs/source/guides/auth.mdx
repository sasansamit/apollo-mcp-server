...
<Callout type="note">
  To use the Authorization feature you'll need to have an Apollo account. We
  offer Free, Developer, Standard, and Enterprise tiers - [learn
  more](https://www.apollographql.com/pricing?referrer=docs-content)
</Callout>
...
<Callout type="warning">
  Known Limitation: The current implementation passes through OAuth tokens from MCP clients directly to upstream GraphQL APIs, which violates the MCP specification. See [Limitations](/apollo-mcp-server/limitations#authorization-token-passthrough) for details and security considerations.
</Callout>
