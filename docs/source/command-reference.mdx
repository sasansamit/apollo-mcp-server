...
These installation methods don't support Intel Macs. Support is on the roadmap.
...
```bash
iwr 'https://mcp.apollo.dev/win/latest' | iex
```
...
```bash
# Note the `v` prefixing the version number
iwr 'https://mcp.apollo.dev/win/v0.1.0' | iex
```
