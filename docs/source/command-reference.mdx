...
In addition to the standalone container we also provide the GraphOS Runtime Container that provides all the necessary components to run the Apollo Runtime, including the Apollo Router and the MCP Server.

For more information, including how to deploy and configure this container, please see the [GraphOS Runtime Container repository](https://github.com/apollographql/graphos-runtime).
...
curl -sSL https://mcp.apollo.dev/download/nix/v0.4.0| sh
```
...
```bash
# Note the `v` prefixing the version number
iwr 'https://mcp.apollo.dev/download/win/v0.4.0' | iex
```
...
```bash
# Note the `v` prefixing the version number
docker image pull ghcr.io/apollographql/apollo-mcp-server:v0.4.0
```
