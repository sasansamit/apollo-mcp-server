...
The Apollo MCP server is configured through a configuration file, the path of which
...
|--:---------------|--:------------------|--:-----------------------|--:------------------------------------------------------------|
| `custom_scalars` | path                |                          | Path to a custom scalar map                                   |
...
| `overrides`      | `Overrides`         |                          | Overrides for server behaviour                                |
...
| Option                    | Type   | Default | Description                                                                                       |
|--:------------------------|--:-----|--:------|--:------------------------------------------------------------------------------------------------|
| `apollo_key`              | string |         | The Apollo GraphOS key. Can also be from the `APOLLO_KEY` environment variable                    |
| `apollo_graph_ref`        | string |         | The Apollo GraphOS graph reference. Can also be from the `APOLLO_GRAPH_REF` environment variable  |
| `apollo_registry_url`     | url    |         | The URL to use for Apollo's registry                                                              |
| `apollo_uplink_endpoints` | url    |         | List of uplink URL overrides. Can also be from the `APOLLO_UPLINK_ENDPOINTS` environment variable |
...
| `introspect`         | object |         | Introspect configuration for allowing clients to run instrospection |
...
These fields are under the top-level `operations` key. There are a few valid choices for this key based on the `source` key
...
| GraphOS Collection | `id`     | string         |         | The collection ID to use in GraphOS. Use `default` for the default collection                |
| Introspection      | `source` | `"introspect"` |         | Load operations by introspecting the schema. Note: This requires introspection to be enabled |
| Local              | `source` | `"local"`      |         | Load operations from local GraphQL files / folders                                           |
| Local              | `paths`  | list<path>     |         | Paths to GraphQL files or folders containing them to search                                  |
| Manifest           | `source` | `"manifest"`   |         | Load operations from a persisted queries manifest file                                       |
| Manifest           | `path`   | path           |         | The path to the persisted query manifest                                                     |
| Uplink             | `source` | `"uplink"`     |         | Load operations from uplink manifest. Note: requires an Apollo key and graph reference       |
...
| Option                       | Type                                | Default  | Description                                                                                                                 |
|--:---------------------------|--:----------------------------------|--:-------|--:--------------------------------------------------------------------------------------------------------------------------|
| `disable_type_description`   | bool                                | `false`  | Disable type descriptions to save on contect-window space                                                                   |
| `disable_schema_description` | bool                                | `false`  | Disable schema descriptions to save on contect-window space                                                                 |
| `enable_explorer`            | bool                                | `false`  | Expose a tool that returns the URL to open a GraphQL operation in Apollo Explorer. Note: requires a GraphOS graph reference |
| `mutation_mode`              | `oneOf ["none", "explicit", "all"]` | `"none"` | Set the mutation mode access level for the MCP server                                                                       |


#### Schema Source Configuration

These fields are under the top-level `schema` key. There are a few valid choices for this key based on the `source` key
with the default being `uplink`.

| Source | Option   | Type       | Default | Description                                                                    |
|--:-----|--:-------|--:---------|--:------|--:-----------------------------------------------------------------------------|
| Local  | `source` | `"local"`  |         | Load schema from local file                                                    |
| Local  | `path`   | path       |         | Path to the GraphQL schema                                                     |
| Uplink | `source` | `"uplink"` | *       | Fetch the schema from uplink. Note: requires an Apollo key and graph reference |


#### Transport Configuration

These fields are under the top-level `transport` key. There are a few valid choices for this key based on the `type` key
with the default being `stdio`.

| Type           | Option    | Type                | Default     | Description                                                                                                                   |
|--:-------------|--:--------|--:------------------|--:----------|--:----------------------------------------------------------------------------------------------------------------------------|
| stdio          | `type`    | `"stdio"`           | *           | Use standard IO for server <> client communication                                                                            |
...
| StreamableHTTP | `type`    | `"streamable_http"` |             | Host the MCP server on the configuration, using stremable HTTP messages.                                                      |
