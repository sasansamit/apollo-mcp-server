...
1. Modify the MCP server configuration in `graphql/TheSpaceDevs/config.yaml` to the following so it points to the `/data` path you'll map things to as expected by the Dockerfile:
