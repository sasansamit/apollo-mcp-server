...
See the [user guide](/apollo-mcp-server/guides) to learn how to create tools from persisted queries with [persisted query manifests](/apollo-mcp-server/guides/#from-persisted-query-manifests) or [GraphOS](/apollo-mcp-server/guides/#from-graphos-managed-persisted-queries).
