...
> **Note**: You'll need Node v18 or later installed for `mcp-remote` to work. If you have an older version of Node, uninstall it and install the latest version from [nodejs.org](https://nodejs.org/).
...
- **Problem**: "MCP thespacedevs: Server disconnected" errors
  - Solution: Uninstall older versions of Node. `mcp-remote` only works with Node 18 or later.
