...
- Install [Apollo Rover CLI](/rover/getting-started) v0.31 or later
...
4. In Inspector:
    ...
    - Enter `http://localhost:5000/sse` as the URL
...
- **Error**: "Port 5000 is already in use"
  - Solution: Kill any existing processes using port 5000 or specify a different port with `--port` flag
...
- **Error**: "Failed to connect to server"
  ...
  - Solution: Verify you're using the correct URL (`http://localhost:5000/sse`)
...
You can run the MCP Server using STDIO transport instead of SSE. This is useful for certain environments or when you need more direct control over the server process.
